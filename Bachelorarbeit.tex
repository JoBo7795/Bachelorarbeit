...
Eine spezielle und eigens für das Projekt entwickelte Komponente ist der \glqq Alert-Action-Manager\grqq, dessen Aufbau und Funktion in Kapitel sieben beschrieben und erklärt.
...
In der Umfrage stellte sich heraus, dass über 78 Prozent der 1337 Befragten Kubernetes verwendet. \cite{cncf2019}\\
...
Die meistverwendete Container-Runtime ist, wie auch in diesem Projekt, Docker. Es gibt aber auch Unterstützung für Weitere wie beispielsweise Rocket. \cite{pod}%\cite{.20200814T19:56:28.000Z}
\item [Deployment]:\\
Das Deployment ist eine Beschreibung des Zustands, in dem Pods ausgeliefert werden sollen. Hier werden beispielsweise der Name, Namensraum, Ressourcenlimits oder auch die Größe der Skalierung definiert. \cite{deployment}%\cite{.13.06.2020}
\item [Service]:\\
Nach der offiziellen Kubernetes-Dokumentation sind Services eine \glqq abstrakte Möglichkeit, eine Anwendung, die auf einer Reihe von Pods läuft, als Netzwerkdienst bereitzustellen\grqq. \cite{service} %\cite{.30.05.2020b}
...
abbildet. Im Buch \glqq IT Service Management: A Guide for ITIL Foundation Exam Candidates\grqq{} werden Metriken \glqq zur kontinuierlichen Verbesserung von Services\grqq{}\cite{Brewster.2012} im Grunde in 3 Bereiche eingeteilt:
\begin{itemize}
\item Service-Metriken, welche die Performance eines Service bemessen, zum Beispiel die
unterbrechungs bzw. ausfallfreie Laufzeit in einem Monat.
\item Prozess-Metriken, die für die Quantifizierung des Entwicklungsprozesses einer Software verwendet werden und um Verbesserungsmöglichkeiten zu identifizieren.
\item Technologie-Metriken, welche die zugrunde liegende Technologie und Komponenten quantifizieren, zum
Beispiel die Speicherauslastung.
\end{itemize} \cite{Brewster.2012}
...
In dieser Arbeit werden Service und Technologie-Metriken erstellt und verwendet, welche die Performance der in einem Kubernetes-Cluster laufenden Services beziffert.
...
Ein Scraping-Target, beispielsweise ein Kubernetes-Node, besitzt einen sog. Exporter, der Metriken aus dem System ausließt und diese an einem HTTP-Endpunkt \begin{verbatim}
IP-Adresse/DNS-Name>/metrics 
\end{verbatim}bereitstellt. Der Prometheus-Server findet entweder per automatischem Target-Disovery Mechanismus das Target oder wird per Konfiguration darauf eingestellt.
...
\begin{quote}\glqq Go is a compiled, concurrent, garbage-collected, statically typed language developed at Google. It is an open source project: Google imports the public repository rather than the other way around.\grqq~\cite{Tien.2019}\end{quote}
...
\captionof{figure}{Formel der CPU-Utilization \cite{metrics2} } 
...
\)
\captionof{figure}{Formel der CPU-Saturation \cite{metrics2} } 
...
\) 
\captionof{figure}{Formel der RAM-Utilization \cite{metrics2} } 
...
\) 
\captionof{figure}{Formel der RAM-Saturation \cite{metrics3} } 
\end{minipage}
...
\) 
\captionof{figure}{Formel der Netzwerk-Utilization \cite{metrics2} } 
\end{minipage}
...
\) 
\captionof{figure}{Formel der Netzwerk-Error-Rate \cite{metrics2} } 
\end{minipage}
...
\begin{minipage}{\linewidth}
  \vspace{-3cm}
  \scalebox{.8}{\input{DatenaggregationFlow (3).pdf_tex}} 
...
\)\\
\captionof{figure}{Formel des \glqq Moving-Average\grqq-Filter \cite{wiki:xx0} }
...
\newpage
\bibliography{ManLiteratur}

\newpage
\includepdf{SCN_0001.pdf}
