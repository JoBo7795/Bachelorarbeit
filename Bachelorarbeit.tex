...
Im ersten Schritt aggregiert der Prometheus-Server aus dem Kubernetes-Cluster Systemressourcen bezogene Metriken von Pods und Containern. 

\item[Schritt 2:]

In Schritt zwei prüft der Prometheus-Server anhand konfigurierter Alerting-Regeln, ob ein Alert ausgelöst werden soll. Falls dies der Fall ist, versendet der Server einen Alert an den Prometheus-Alertmanager, zuerst mit dem Status 'pending', nach Ablauf der Offset-Zeit mit dem Status 'active' (siehe Kapitel Grundlagen/Alerting).

\item[Schritt 3:]

Der Alertmanager versendet, sobald ein Alert den Status 'active' erhält, an alle konfigurierten Ziele eine Alert-Nachricht. Diese sind ein Channel des Chatprogramms 'Slack', der alleine für Alert-Nachrichten zur Verfügung steht und den Zweck besitzt Administratoren und andere Interessierte zu informieren, sowie das in diesem Projekt entwickelte Tool 'Alert-Action-Manager', welches den Zweck besitzt anhand eintreffender Alert-Nachrichten Aktionen auf das Kubernetes-Cluster auszuführen.

\item[Schritt 4:]

Beim Eintreffen von Alerts prüft der 'Alert-Action-Manager' die eingehenden Nachrichten auf: 

\begin{description}

\item[1.] auf ihr Label, um den Grund sowie die daraus resultierende Aktion eines Alerts schließen zu können. Zum Beispiel wird auf das Label 'PodLowResource' mit der Upscaling-Aktion reagiert.
\item[2.] auf ihr Deployment, von welcher Ressource der Alert kommt und wohin die resultierende Aktion ausgeführt werden soll. Je nach Aktion kann der Alert-Action-Manager diese entweder auf das ganze Deployment oder auf einzelne Pods ausführen.
\item[3.] auf benötigte weitere Daten. Im Falle einer Aktion können weitere Daten benötigt werden, wie beispielsweise im Falle eines Scalings, der 'Replica-Count', also die Anzahl der bereits vorhandenen Pods eines Deployments im Cluster. Solche Informationen können einerseits entweder in einen Alert geschrieben, oder in einer Aktion durch den Manager ermittelt werden.

\end{description}

\item[Schritt 5:]

Nachdem eine Nachricht durch den Alert-Action-Manager geprüft wurde, wird eine entsprechende Aktion auf das Kubernetes-Cluster ausgeführt. Dies geschieht mittels des Kubernetes Kontrolltools 'kubectl'.\\
Nach dem Ausführen von Schritt fünf ist der Vorgang abgeschlossen.
\end{description}
...
Der Alertmanager wird zusammen mit Prometheus über den 'Helm' Paket-Manager installiert und auch über dessen Helm-Charts konfiguriert.
...
Nach Auslösen des Alerts und Aktivieren der Aktion im' Alert-Action-Manager' setzt dieser die Aktion folgendermaßen um:
...
\begin{lstlisting}
kubectl scale deployment deployment_name --replicas= strconv.Itoa(actualCount+1)
\end{lstlisting}
